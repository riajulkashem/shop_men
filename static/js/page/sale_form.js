$(document).ready(function () {
    ...
    function init_plugin() {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data['error']) {
                        ...
                        $('input[name="shopping_products-' + form_number + '-price"]').attr('min', data['buy_price']).val(data['sell_price'])
                        $('input[name="shopping_products-' + form_number + '-quantity"]').attr('max', data['stock'])
                    }
                },
        ...
        $('.price').on('keyup change', function () {
            let form_number = parseInt($(this).attr('name').replace(/[^0-9.]/g, ""));
            let purchase_price = $(this).attr('min')
            if (parseFloat($(this).val()) < parseFloat(purchase_price)) $(this).val(purchase_price)
        ...
        $('.quantity').on('keyup change', function () {
            ...
            if (parseInt($(this).val()) > parseInt(stock)) {
                $(this).val(stock)
                toastr.warning('Product Stock not available more than ' + stock, NOTIFICATION_TITLE)
            }
            calculate_total(form_number)
        })
